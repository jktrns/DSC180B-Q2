...
\end{tcolorbox}
\caption{Condensed PE prompt sent to GPT-5 nano. The full prompt lists all 68 fields and the top-10 values for each categorical column. Each API call requests 20 records and enforces schema compliance via a Pydantic \texttt{json\_schema} structured output constraint, so the model cannot hallucinate field names or produce malformed types.}
\label{fig:pe-prompt}
\end{figure}
...
\bottomrule
\endlastfoot
\texttt{XeonNet} & Agg & 96.5 & \textbf{6.5} & 593.4 & 5.1 \\
\texttt{ChassisAgg} & Agg & 309.3 & 4.0 & 4.0 & \textbf{3.3} \\
\texttt{BattDuration} & Agg & --- & \textbf{12.6} & 248.1 & --- \\
\texttt{BattGeo} & Agg & 139.1 & \textbf{2.5} & 320.5 & 8.3 \\
\texttt{DisplayDurations} & Agg & --- & \textbf{13.8} & 3{,}850.4 & --- \\
\texttt{DisplayVendors} & Dist & --- & 44.6 & \textbf{27.5} & --- \\
\texttt{ModsBlockers} & Agg & --- & \textbf{16.1} & 35.3 & --- \\
\texttt{BrowserCountry} & Cat & 9.0 & 4.0 & \textbf{3.0} & 38.0 \\
\texttt{OnOffMods} & Agg & --- & \textbf{19.7} & 8{,}655.3 & --- \\
\texttt{PersonaWeb} & Agg & 65.4 & \textbf{3.4} & 35.3 & 41.2 \\
\texttt{PkgPower} & Agg & 616.6 & 3.3 & \textbf{1.0} & 8.3 \\
\texttt{BrowserHist} & Dist & 95.5 & 118.0 & \textbf{25.5} & 65.1 \\
\texttt{RamHist} & Hist & 29.8 & 38.3 & 110.9 & \textbf{14.5} \\
\texttt{ServerExpl} & Row & --- & \textbf{35.3} & 46.9 & --- \\
\texttt{AppFocal} & Rank & --- & \textbf{1.0} & 1.1 & --- \\
\texttt{AppSysCount} & Rank & --- & 45.9 & \textbf{9.7} & --- \\
\texttt{AppDetections} & Rank & --- & 4.7 & \textbf{2.0} & --- \\
\texttt{ModsBlockerDur} & Agg & --- & \textbf{2.4} & 45.1 & --- \\
\texttt{UserWait10} & Rank & --- & \textbf{1.0} & \textbf{1.0} & --- \\
\texttt{UserWaitACDC} & Rank & --- & \textbf{0.9} & \textbf{0.9} & --- \\
\end{longtable}
\end{footnotesize}

The per-query breakdown reveals several patterns. First, per-table DP-SGD achieves normalized errors below 5$\times$ on 7 of 20 queries, indicating that for over a third of the benchmark its synthesis error is within a small multiple of natural sampling variation. Second, MST excels on ranking queries (\texttt{UserWait*}, \texttt{AppDetections}) where it achieves near-unity normalized errors, confirming its strength in preserving marginal orderings. Third, the wide-table methods show extreme normalized errors on aggregate queries (e.g., \texttt{PkgPower} at 617$\times$, \texttt{ChassisAgg} at 309$\times$), driven by the zero-inflation problem discussed in Section~\ref{sec:methods-wide-dpsgd}. Fourth, PE achieves competitive normalized errors on the queries it can evaluate (e.g., \texttt{ChassisAgg} at 3.3, \texttt{XeonNet} at 5.1) but covers only 8 of 20 queries.
