...
As discussed in Section~\ref{sec:normalized-error}, we propose a \textit{normalized error} metric (Equation~\ref{eq:normalized-error}) that divides each query's discrepancy by the baseline variation $|\Delta_j(D_1, D_2)|$ obtained from a random 50/50 split of the real data. Computing this baseline requires re-running all benchmark queries on the split data, which we defer to a follow-up evaluation pass. In the current results, we report binary pass rates at the thresholds $\text{RE} \leq 0.25$, $\text{TV} \leq 0.15$, and $\rho \geq 0.5$. The query score is the fraction of metric columns that pass. A query is marked as passing when its score is at least 0.5, with query-specific overlap thresholds defined in the evaluation metadata. We test threshold sensitivity in Figure~\ref{fig:sensitivity} to verify that comparative conclusions are not driven by a single cutoff choice.
