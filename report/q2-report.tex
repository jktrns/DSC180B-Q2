...
\end{equation}
where the denominator $|\Delta_j(D_1, D_2)|$ is the natural sampling variation.\footnote{When $|\Delta_j(D_1, D_2)| = 0$ (the two halves produce identical results), the normalized error is undefined. We exclude such metrics rather than using a small stabilizer $\eta$, since an $\eta$-stabilized ratio (e.g., $0.5/10^{-8} = 5 \times 10^7$) would dominate summary statistics. This affects 18 of 282 metric measurements (6.4\%), primarily Jaccard group-coverage scores.} Intuitively, $\hat{\Delta}_j = 1$ means the synthetic error equals the variation from halving the real dataset; $\hat{\Delta}_j < 1$ means better-than-half-sample fidelity; $\hat{\Delta}_j \gg 1$ means the synthesis error far exceeds natural variation. The aggregate benchmark score is:
\begin{equation}
...
Figure~\ref{fig:training-curves} plots the training loss and cumulative privacy budget ($\varepsilon$) over the 20 epochs of DP-SGD training. Despite the noisier gradients from per-sample clipping, the training loss decreases monotonically from 23.76 to 17.31 (a 27\% reduction), with most improvement in the first 5 epochs and diminishing returns thereafter. The cumulative privacy spend increases roughly linearly, reaching $\varepsilon = 3.996$ at epoch 20, just under the target budget.
...
\end{tikzpicture}
\end{minipage}
\caption{Wide-table DP-VAE training dynamics over 20 epochs ($\varepsilon_{\text{target}} = 4.0$, $\delta = 10^{-5}$). Left: training loss decreases monotonically with most improvement in early epochs. Right: cumulative privacy budget grows approximately linearly, reaching $\varepsilon = 3.996$ at convergence.}
...
The synthetic wide table is then decomposed back into 13 reporting table schemas by selecting the relevant columns for each table and assigning synthetic \texttt{guid}s. The original benchmark SQL queries execute unchanged on these synthetic reporting tables.

We evaluate 10 of the 21 feasible queries whose reporting tables are fully reconstructable from the wide-table columns. The remaining 11 queries require per-row columns not present in the wide table (e.g., per-process breakdowns for userwait, per-application data for foreground apps, per-display data for display devices).
...
\begin{enumerate}
    ...
    \item Add calibrated Gaussian noise to every aggregate query via the Gaussian mechanism, spending a privacy budget of $\varepsilon_{\text{agg}} = 3.5$. We employ zero-concentrated differential privacy (zCDP) composition \citep{bun2016concentrated}, splitting the budget equally across 7 query groups (battery, memory, on/off, browser/webcat, hardware, network, and hints).\footnote{Converting the total $(\varepsilon_{\text{agg}}, \delta)$ budget to a zCDP parameter $\rho_{\text{total}}$ and splitting $\rho$ equally yields per-query noise scales $\sigma_g = \sqrt{k_g / (2\rho_g)}$ ranging from 24.6 (network, $k=40$ queries) to 68.2 (browser/webcat, $k=308$ queries), substantially tighter than the $\sigma \approx 150$ from naive advanced composition across all 1{,}056 queries jointly.}
...
\begin{enumerate}
    ...
    \item \textit{Synthetic generation via Private-PGM.} The noisy marginals define a tree-structured graphical model. MST uses the Private-PGM algorithm \citep{mckenna2019graphical} to fit a distribution maximally consistent with all noisy marginals and samples $n$ synthetic records from it.\footnote{Private-PGM solves this via iterative proportional fitting on the junction tree. The sparse tree structure ensures tractable local computations even for tables with many columns.}
\end{enumerate}
...
We evaluate the wide-table DP-VAE synthetic data on 10 of 21 feasible benchmark queries. The remaining 11 queries require per-row reporting columns that are not represented in the wide table. The model passes 1 of 10 queries with average score 0.206 (Table~\ref{tab:method-comparison}).
...
\texttt{SleepPivot} & Pivot & \cellcolor{scorelo}0.000 \\
\midrule
...
Per-table DP-SGD passes 6 of 21 queries, with average score 0.303 and median score 0.250. Unlike the wide-table methods (DP-VAE and PE on 10 queries each), per-table DP-SGD is evaluated on the full 21-query benchmark. Table~\ref{tab:pertable-fidelity} reports all 21 per-query scores.
...
MST passes 6 of 21 queries, with average score 0.328 and median score 0.250. Like per-table DP-SGD, MST is evaluated on the full 21-query benchmark. Table~\ref{tab:mst-fidelity} reports all 21 per-query scores.
...
The gains from conditional generation concentrate on queries whose structure aligns with the stratification (e.g., on/off/sleep and RAM utilization), while misaligned queries see minimal improvement or slight regression.\footnote{Browser popularity drops from 0.333 to 0.000 because the reduced histogram budget ($\sigma = 7.03$ vs.\ $1.08$) weakens correction of the model's browser distribution bias. The chassis power query regresses because the hardware stratum is the smallest (55 records from $k=48$ queries), producing zero overlapping groups.} Country-stratified queries remain at zero for both variants because per-country populations (typically 1--128 systems) fall below the DP noise floor.
...
\midrule
Wide-table DP-SGD & 10 & 1 & 10.0\% & 0.206 & 0.116 \\
...
Table~\ref{tab:normalized-error} reports the normalized error metric (Equation~\ref{eq:normalized-error}) using the methodology described in Section~\ref{sec:normalized-error}.
...
Wide-table DP-VAE & 517.78 & 15.34 & 3{,}105.00 & 55 \\
...
Per-table DP-SGD achieves the best median normalized error (2.93$\times$), followed by conditional PE (3.79$\times$), MST (10.76$\times$), and wide-table DP-VAE (15.34$\times$). This ranking differs from the binary pass-rate ranking, where per-table and MST were tied. MST's high mean is driven by extreme outliers on multi-table joins (e.g., \texttt{SleepPivot} at 11{,}711$\times$, \texttt{DisplayAgg} at 3{,}927$\times$).
...
\caption{Per-query mean normalized error ($\hat{\Delta}$, lower is better). Bold indicates best per row. ``---'' means the method did not produce results for that query. $\dagger$Zero group overlap: the only finite metric is Jaccard = 0; all numeric columns have infinite normalized error and are excluded.}
...
\bottomrule
\endlastfoot
\texttt{BlockerAgg} & Agg & --- & \textbf{22.7} & 60.7 & --- \\
\texttt{BlockerDurAgg} & Agg & --- & \textbf{2.4} & 44.9 & --- \\
\texttt{ChassisAgg} & Agg & 4{,}650.3 & --- & --- & --- \\
\texttt{DisplayAgg} & Agg & --- & \textbf{15.7} & 3{,}927.2 & --- \\
\texttt{NetAsymAgg} & Agg & --- & \textbf{28.3} & 34.2 & --- \\
\texttt{VendorAgg} & Agg & --- & 42.3 & \textbf{26.1} & --- \\
\midrule
\texttt{BatteryDemo} & Geo & 0.0$^\dagger$ & 17.5 & 477.6 & \textbf{10.9} \\
\texttt{BatteryGeo} & Geo & 282.4 & \textbf{4.1} & 344.2 & 14.5 \\
\texttt{PowerGeo} & Geo & 634.1 & 2.8 & \textbf{0.9} & 3.6 \\
\texttt{XeonGeo} & Geo & 50.2 & 4.3 & 439.4 & \textbf{3.5} \\
\midrule
\texttt{BrowserHist} & Hist & 115.3 & 91.6 & \textbf{50.1} & 112.8 \\
\texttt{RamHist} & Hist & 30.2 & 32.8 & 91.7 & \textbf{1.5} \\
\midrule
\texttt{PersonaPivot} & Pivot & 54.9 & \textbf{3.3} & 32.0 & 9.2 \\
\texttt{SleepPivot} & Pivot & 0.0$^\dagger$ & 24.3 & 11{,}710.6 & \textbf{9.5} \\
\midrule
\texttt{BrowserRank} & Top-$k$ & 9.0 & 4.0 & \textbf{3.0} & 43.0 \\
\texttt{DetectionRank} & Top-$k$ & --- & 5.4 & \textbf{2.3} & --- \\
\texttt{FocalRank} & Top-$k$ & --- & \textbf{1.0} & 1.1 & --- \\
\texttt{SystemRank} & Top-$k$ & --- & 43.5 & \textbf{5.0} & --- \\
\texttt{WaitModeRank} & Top-$k$ & --- & \textbf{0.9} & \textbf{0.9} & --- \\
\texttt{WaitRank} & Top-$k$ & --- & \textbf{1.0} & \textbf{1.0} & --- \\
\end{longtable}
\end{footnotesize}

The per-query breakdown reveals that per-table DP-SGD achieves normalized errors below 5$\times$ on half the benchmark, conditional PE achieves the lowest error on 4 of its 9 evaluated queries (driven by stratification alignment), and MST excels on ranking queries with near-unity errors. The wide-table methods show extreme errors on aggregate queries (e.g., \texttt{ChassisAgg} at 4{,}650$\times$), consistent with the zero-inflation problem discussed in Section~\ref{sec:methods-wide-dpsgd}.
...
\midrule
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 \\
...
\texttt{SleepPivot} & Pivot & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.636 & \cellcolor{scoremid}0.273 & \cellcolor{scorehi}0.818 \\
\midrule
...
\addplot[fill=RoyalBlue!40, draw=RoyalBlue, bar shift=0pt] coordinates {(Wide,0.206)};
...
\end{tikzpicture}
\end{minipage}
\caption{Left: number of queries passing (score $\geq 0.5$) for each method. Right: average score across all evaluated queries. The per-table methods evaluate all 21 queries; the wide-table method and PE (conditional) each evaluate 10.}
...
\addplot[fill=RoyalBlue!40, draw=RoyalBlue] coordinates {
    (Agg+Join,0.167) (Top-$k$,1.000) (Geo/Demo,0.208) (Histogram,0.000) (Pivot,0.033)
};
...
The calibration patterns differ by method. Wide-table DP-SGD collapses toward near-zero synthetic outputs regardless of real magnitude. Per-table DP-SGD and PE both show horizontal banding (outputs plateau rather than tracking real values). MST has wider spread but follows the diagonal more closely, consistent with higher pointwise noise but better overall calibration.
...
The relative error threshold has the largest effect: per-table DP-SGD's pass rate rises from 14.3\% at RE $\leq 0.05$ to 57.1\% at RE $\leq 1.0$, indicating many queries land in the right order of magnitude but outside the strict 25\% tolerance. The wide-table method is flat at 10.0\% until RE $\leq 1.0$, confirming that its continuous metric failures are total rather than marginal. Total variation and Spearman $\rho$ thresholds have minimal effect, as queries tend to either match closely or fail badly with few in the boundary region.\footnote{MST achieves $\rho = 1.0$ on its passing ranking queries and near zero on failing ones, so varying the $\rho$ threshold changes nothing. Similarly, distribution queries cluster at TV $< 0.10$ or TV $> 0.5$.}
...
\addplot[RoyalBlue, mark=*] coordinates {
    (0.05,10.0) (0.10,10.0) (0.15,10.0) (0.20,10.0)
    (0.25,10.0) (0.50,10.0) (0.75,10.0) (1.00,60.0)
};
\addplot[BrickRed, mark=square*] coordinates {
...
\addplot[RoyalBlue, mark=*] coordinates {
    (0.05,10.0) (0.10,10.0) (0.15,10.0) (0.20,10.0)
    (0.25,20.0) (0.30,20.0) (0.40,20.0) (0.50,30.0)
};
\addplot[BrickRed, mark=square*] coordinates {
...
\addplot[RoyalBlue, mark=*] coordinates {
    (0.0,10.0) (0.1,10.0) (0.2,10.0) (0.3,10.0) (0.4,10.0)
    (0.5,10.0) (0.6,10.0) (0.7,10.0) (0.8,10.0) (0.9,10.0)
};
\addplot[BrickRed, mark=square*] coordinates {
...
All four methods have completed evaluation against the 21-query benchmark. Under binary pass-rate scoring, per-table DP-SGD and MST each pass 6 of 21 queries (28.6\%), the wide-table DP-VAE passes 1 of 10 (10.0\%), and conditional PE passes 3 of 10 (30.0\%). The normalized error metric (Section~\ref{sec:normalized-error}), which calibrates each query's synthesis error against the natural variation from a 50/50 data split, provides further differentiation across all four methods: per-table DP-SGD achieves a median normalized error of 2.93$\times$, meaning its typical distortion is under 3 times the variation expected from halving the real dataset, followed by conditional PE at 3.79$\times$, MST at 10.76$\times$, and wide-table DP-VAE at 15.34$\times$. Per-table DP-SGD achieves normalized errors below 5$\times$ on 10 of 20 queries, while conditional PE achieves the lowest normalized error on 4 of its 9 evaluated queries, including RAM utilization (1.5$\times$), Xeon network consumption (3.5$\times$), sleep pivot (9.5$\times$), and battery demographics (10.9$\times$). MST excels specifically on ranking queries (achieving near-unity normalized errors on \texttt{WaitRank} and \texttt{WaitModeRank}) but suffers extreme errors on multi-table aggregate joins.
