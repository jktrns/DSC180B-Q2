...
\bottomrule
\endlastfoot
\texttt{XeonGeo} & Geo & 96.5 & 6.5 & 593.4 & \textbf{5.1} \\
\texttt{ChassisAgg} & Agg & 309.3 & 4.0 & 4.0 & \textbf{3.3} \\
\texttt{BatteryDemo} & Geo & --- & \textbf{12.6} & 248.1 & --- \\
\texttt{BatteryGeo} & Geo & 139.1 & \textbf{2.5} & 320.5 & 8.3 \\
...
\texttt{PowerGeo} & Geo & 616.6 & 3.3 & \textbf{1.0} & 8.3 \\
\texttt{BrowserHist} & Hist & 95.5 & 118.0 & \textbf{25.5} & 65.1 \\
\texttt{RamHist} & Hist & 29.8 & 38.3 & 110.9 & \textbf{14.5} \\
...
\end{tikzpicture}
\end{minipage}
\caption{Left: number of queries passing (score $\geq 0.5$) for each method. Right: average score across all evaluated queries. The per-table methods evaluate all 21 queries, while the wide-table and PE methods evaluate 8.}
...
The third intervention redesigns the PE generation pipeline to condition on aggregate statistics derived from the real query results. Instead of generating records independently (as in the baseline PE), we:
\begin{enumerate}
...
\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.50\textwidth}YYY}
\toprule
\textbf{Query} & \textbf{PE baseline} & \textbf{Stratified PE} & \textbf{Delta} \\
\midrule
ram\_utilization\_histogram & 0.000 & \cellcolor{scorehi}1.000 & +1.000 \\
popular\_browsers\_by\_count\_usage\_pct & \cellcolor{scorehi}0.667 & 0.000 & $-$0.667 \\
Xeon\_network\_consumption & 0.250 & 0.000 & $-$0.250 \\
on\_off\_mods\_sleep\_summary & 0.091 & \cellcolor{scorehi}0.636 & +0.545 \\
persona\_web\_cat\_usage\_analysis & 0.032 & 0.097 & +0.065 \\
battery\_on\_duration\_cpu\_family\_gen & 0.000 & 0.333 & +0.333 \\
battery\_power\_on\_geographic\_summary & 0.000 & 0.000 & 0.000 \\
avg\_platform\_power\_c0\_freq\_temp\_by\_chassis & 0.000 & 0.000 & 0.000 \\
most\_popular\_browser\_in\_each\_country & 0.000 & 0.000 & 0.000 \\
pkg\_power\_by\_country & 0.000 & 0.000 & 0.000 \\
\midrule
\textbf{Passed ($\geq 0.5$)} & \textbf{1/10} & \textbf{2/10} & \\
\textbf{Average score} & \textbf{0.104} & \textbf{0.207} & \\
\bottomrule
\end{tabularx}
\caption{Stratified PE with DP-noised aggregates ($\varepsilon_{\text{agg}}=1.0 + \varepsilon_{\text{hist}}=3.0 = 4.0$ total). The ``PE baseline'' column reports the midscale PE run (5{,}000 records, independent generation, same $\varepsilon=4.0$). Stratified generation improves on RAM utilization and on/off/sleep queries where per-stratum conditioning aligns with query structure, but regresses on popular browsers ($0.667 \to 0.000$) and Xeon network ($0.250 \to 0.000$) because the high per-query noise ($\sigma \approx 150$) in v1 destroys the aggregate targets for those groups.}
\label{tab:stratified-pe}
\end{table}

The baseline PE midscale run (5{,}000 records, independent generation at $\varepsilon = 4.0$) passes 1 of 10 evaluable queries with an average score of 0.104, with its only passing query being popular browsers (0.667). The stratified v1 approach passes 2 of 10 queries with an average score of 0.207. The gains concentrate on queries whose structure matches the stratification: RAM utilization (stratified by memory size) achieves a perfect 1.0, and the on/off/sleep summary (stratified by CPU market code name) passes at 0.636. However, the stratified v1 approach regresses on popular browsers ($0.667 \to 0.000$) and Xeon network consumption ($0.250 \to 0.000$) because the high per-query noise ($\sigma \approx 150$) from composing all $k$ aggregate queries under a single budget destroys the aggregate targets for those groups. Country-stratified queries remain at 0 because the DP noise on per-country counts (sensitivity = 1 per system) overwhelms the small per-country populations, producing effectively uniform country distributions in the prompts.
...
\begin{enumerate}
    \item \textbf{Per-group budget splitting (Idea \#1):} Instead of composing all $k=1{,}056$ aggregate queries under a single $\varepsilon_{\text{agg}}$, we partition queries into 7 groups (battery, memory, on/off, browser/webcat, hardware, network, hints) and allocate each group an equal share of the total zCDP $\rho$ budget.
    \item \textbf{zCDP composition (Idea \#2):} We replace basic advanced composition with zero-concentrated DP~\cite{bun2016concentrated}. We convert the total $(\varepsilon_{\text{agg}}, \delta)$ budget to a zCDP parameter $\rho_{\text{total}}$ via $\varepsilon = \rho + 2\sqrt{\rho \ln(1/\delta)}$, split $\rho$ equally across groups, then derive per-query $\sigma_g = \sqrt{k_g / (2\rho_g)}$ for each group $g$ with $k_g$ queries.
    \item \textbf{Budget rebalancing (Idea \#3):} We shift budget from the histogram to aggregates: $\varepsilon_{\text{agg}} = 3.5$, $\varepsilon_{\text{hist}} = 0.5$ (total unchanged at 4.0). Since stratified generation already conditions the pool on targets, the histogram adds less marginal value than accurate aggregates.
    \item \textbf{Post-processing consistency (Idea \#6):} After noising, webcat percentage targets are normalized to sum to 100 and all noisy counts are clamped to $[0, \infty)$. This is free under DP (deterministic post-processing).
\end{enumerate}
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.42\textwidth}YYYY}
\toprule
\textbf{Query} & \textbf{PE base} & \textbf{Strat v1} & \textbf{Strat v2} & \textbf{$\Delta$} \\
\midrule
ram\_utilization\_histogram & 0.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & 0.000 \\
popular\_browsers\_by\_count\_usage\_pct & \cellcolor{scorehi}0.667 & 0.000 & 0.333 & +0.333 \\
on\_off\_mods\_sleep\_summary & 0.091 & \cellcolor{scorehi}0.636 & \cellcolor{scorehi}0.636 & 0.000 \\
Xeon\_network\_consumption & 0.250 & 0.000 & \cellcolor{scorehi}0.500 & +0.500 \\
battery\_on\_duration\_cpu\_family\_gen & 0.000 & 0.333 & 0.333 & 0.000 \\
persona\_web\_cat\_usage\_analysis & 0.032 & 0.097 & 0.161 & +0.064 \\
battery\_power\_on\_geographic\_summary & 0.000 & 0.000 & 0.000 & 0.000 \\
avg\_platform\_power\_c0\_freq\_temp\_by\_chassis & 0.000 & 0.000 & 0.000 & 0.000 \\
most\_popular\_browser\_in\_each\_country & 0.000 & 0.000 & 0.000 & 0.000 \\
pkg\_power\_by\_country & 0.000 & 0.000 & 0.000 & 0.000 \\
\midrule
...
The improved calibration lifts performance from 2/10 (v1) to 3/10 (v2) passed queries (average 0.207 $\to$ 0.296). The newly passing query is Xeon network consumption ($0.500$), which benefits directly from the reduced $\sigma_{\text{net}}=24.6$ (vs.\ $\sigma=150$ in v1). Browser popularity also recovers to 0.333 in v2 (compared to 0.667 in the independent baseline and 0.000 in v1). Country-stratified queries remain at 0: even with $\sigma \approx 65$ for the hints group, the per-country populations (1--128 systems) are still below the noise floor. Overall, stratified PE v2 passes 3/10 queries at an average score of 0.296, compared to the independent baseline's 1/10 at 0.104. This confirms that DP noise calibration is the binding constraint on stratified PE utility, and further gains would require either reducing the number of queries per group or fundamentally different approaches to country-level conditioning.
