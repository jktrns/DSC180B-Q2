...
We adopt this normalized error framework for future evaluation. Computing the baseline $\Delta_j(D_1, D_2)$ requires running each benchmark query on two independent half-samples of the real dataset; we leave this computation to a follow-up pass when the full telemetry data is available on the evaluation host. In the current results, we report binary pass rates at the thresholds $\text{RE} \leq 0.25$, $\text{TV} \leq 0.15$, and $\rho \geq 0.5$, which should be interpreted as coarse summaries. The normalized error metric will provide a more granular ranking once the $D_1/D_2$ baselines are computed.
...
As discussed in Section~\ref{sec:normalized-error}, we additionally propose a \textit{normalized error} metric (Equation~\ref{eq:normalized-error}) that divides each query's discrepancy by the baseline variation $|\Delta_j(D_1, D_2)|$ from a random 50/50 split. Computing this denominator requires re-running all benchmark queries on two independent half-samples of the real data; we leave this normalization to a follow-up pass and report unnormalized continuous metrics here. We test threshold sensitivity in Figure~\ref{fig:sensitivity} to verify that comparative conclusions are not driven by a single cutoff choice.
...
All four methods have completed evaluation against the 21-query benchmark. Per-table DP-SGD and MST each pass 6 of 21 queries (28.6\% pass rate), and they pass different subsets that reflect their algorithmic strengths. DP histogram synthesis preserves continuous distributions (on-time RE = 0.13), while MST preserves rankings (Spearman $\rho$ = 1.00 on application counts). The wide-table DP-VAE passes 1 of 8 evaluated queries (12.5\%), with all continuous metrics near zero due to extreme sparsity in the wide-table representation. PE passes 2 of 8 (25.0\%), outperforming the wide-table DP-VAE in pass count but with a lower average score (0.150 vs.\ 0.258). PE's two passing queries (\texttt{BrowserHist} and \texttt{RamHist}) are histogram-type queries where the DP nearest-neighbor selection can correct the foundation model's distributional biases.
