...
\end{equation}
where the denominator $|\Delta_j(D_1, D_2)|$ is the natural sampling variation.\footnote{When $D_1$ and $D_2$ produce identical query results for a metric column ($|\Delta_j(D_1, D_2)| = 0$), the normalized error is undefined. We exclude such metrics (22 of 284 method--metric pairs, or 7.7\%, primarily Jaccard group-coverage scores where the two random halves produce identical group-key sets) rather than using a stabilizer $\eta$, since an $\eta$-stabilized ratio would produce astronomically large values that dominate summary statistics.} A normalized error of $\hat{\Delta}_j = 1$ means the synthetic error equals the variation expected from halving the real dataset; $\hat{\Delta}_j < 1$ means the synthetic data is closer to the full real data than a random half-sample; $\hat{\Delta}_j \gg 1$ means the synthesis error far exceeds natural variation. The aggregate benchmark score is:
\begin{equation}
...
Figure~\ref{fig:training-curves} plots the training loss and cumulative privacy budget ($\varepsilon$) over the 20 epochs of DP-SGD training. The training loss decreases monotonically from 23.76 to 17.31, with the steepest improvement in the first 5 epochs. The cumulative privacy spend increases roughly linearly, reaching $\varepsilon = 3.996$ at epoch 20.
...
\begin{figure}[H]
\centering
\begin{tcolorbox}[colback=white, colframe=black!40, sharp corners, boxrule=0.4pt, fontupper=\scriptsize\ttfamily, title={\scriptsize\bfseries PE prompt (condensed)}, left=2mm, right=2mm, top=1mm, bottom=1mm]
Generate exactly 20 synthetic telemetry records.\\[2pt]
Schema:\\
...
\end{tcolorbox}
\caption{Condensed PE prompt sent to GPT-5 nano. The full prompt lists all 68 fields and the top-10 values for each categorical column. Each API call requests 20 records and enforces schema compliance via a Pydantic \texttt{json\_schema} structured output constraint, so the model cannot produce invalid field names, out-of-vocabulary categorical values, or malformed types.}
\label{fig:pe-prompt}
\end{figure}
...
\begin{figure}[H]
\centering
\begin{tcolorbox}[colback=white, colframe=black!40, sharp corners, boxrule=0.4pt, fontupper=\scriptsize\ttfamily, left=2mm, right=2mm, top=1mm, bottom=1mm]
\{\\
...
\}
\end{tcolorbox}
\caption{Example synthetic record generated by GPT-5 nano. This record has nonzero values only in the \texttt{mem} and \texttt{browser}/\texttt{webcat} groups (2 of 8 groups active), consistent with the sparsity rule. All 68 fields are present and type-correct due to the structured output constraint.}
\label{fig:pe-record}
\end{figure}

The key quality concern is that the foundation model's prior over categorical values diverges from the real distribution. For example, GPT-5 nano generates Windows 11 at 77.8\% vs.\ the real prevalence of 10.4\%, and over-represents ``United States'' and ``Notebook'' categories. The DP nearest-neighbor histogram (Step 2 of the algorithm) partially corrects these biases by upweighting candidates that resemble real records, but cannot compensate when entire regions of the feature space are absent from the candidate pool.
...
MST requires all columns to have finite cardinality. We discretize continuous columns into 20 quantile bins (computed on nonzero values only, with a dedicated zero bin when $> 5\%$ of values are zero) and cap categoricals at 50 categories. After synthesis, continuous values are reconstructed by sampling uniformly within each bin's range.

We use the \texttt{MSTSynthesizer} class from SmartNoise Synth (v1.0.6) with \texttt{epsilon=4.0}, \texttt{delta=1e-5}, and the default selection/measurement budget split. Like the per-table DP-SGD approach, MST synthesizes each of the 19 reporting tables independently, with the total privacy budget under basic composition at $19 \times 4.0 = 76.0$. The full 19-table pipeline completes in approximately 90 minutes.
...
PE remains weak on aggregate and geographic queries. The structured output constraint eliminates out-of-vocabulary categorical values, but coverage gaps persist: PE's candidate pool under-represents rare countries and demographic segments because the foundation model's prior favors common categories. \texttt{PersonaPivot} also remains low (0.032) because the persona distribution in PE output diverges from the real distribution.

\subsubsection{Conditional PE with DP-noised aggregates}

We call the following approach \textit{conditional PE} because the generation prompt is conditioned on per-stratum aggregate targets, as opposed to the unconditional baseline that generates records from a generic schema description alone.

Because the conditional PE pipeline generates per-table reporting tables (not just a single wide table), it can be evaluated on 10 of the 21 benchmark queries---the 8 available to the baseline PE plus \texttt{BatteryDemo} and \texttt{SleepPivot}, which require dedicated reporting tables. The ``PE base'' column in the tables below re-evaluates the independent PE generation on this 10-query subset for a fair comparison.

To improve PE utility, we redesign the generation pipeline to condition on aggregate statistics derived from the real query results. Instead of generating records independently (as in the baseline PE), we:
\begin{enumerate}
    \item Read aggregate statistics per stratum from the real benchmark CSVs (e.g., average battery duration per CPU generation, RAM utilization per memory size, webcat distributions per persona).
    \item Add calibrated Gaussian noise to every aggregate query via the Gaussian mechanism, spending a privacy budget of $\varepsilon_{\text{agg}} = 1.0$. Per-query noise is calibrated via advanced composition: $\sigma_{\text{per-query}} = \sqrt{2k\ln(1.25/\delta)}/\varepsilon_{\text{agg}}$ where $k = 1{,}056$ is the total number of aggregate queries across all CSVs.
    \item Use the noisy aggregates to construct per-stratum prompts (e.g., ``generate 55 Notebook records where batt\_duration\_mins averages ${\sim}187$''). Since these prompts are deterministic functions of the DP-released aggregates, this step is free post-processing under DP.
    \item Generate a $3\times$ candidate pool (15{,}000 records across 104 strata) and select 5{,}000 via the DP nearest-neighbor histogram mechanism, spending $\varepsilon_{\text{hist}} = 3.0$.
\end{enumerate}

The total privacy cost is $\varepsilon_{\text{agg}} + \varepsilon_{\text{hist}} = 4.0$ by basic composition, matching the baseline PE budget. The key insight is that noising the aggregates before embedding them in prompts restores the independence assumption required for the histogram's sensitivity-1 analysis: since the candidate pool depends only on the DP-released aggregates (not the raw data), changing one record in the private dataset changes at most one vote in the histogram.

\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.26\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}YYY}
\toprule
\textbf{Query} & \textbf{Type} & \textbf{PE base} & \textbf{Cond v1} & \textbf{$\Delta$} \\
\midrule
\texttt{ChassisAgg} & Agg+Join & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\midrule
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & +0.333 \\
\texttt{BatteryGeo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\texttt{PowerGeo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\texttt{XeonGeo} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scorelo}0.000 & $-$0.250 \\
\midrule
\texttt{BrowserHist} & Histogram & \cellcolor{scorehi}0.667 & \cellcolor{scorelo}0.000 & $-$0.667 \\
\texttt{RamHist} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}1.000 & +1.000 \\
\midrule
\texttt{PersonaPivot} & Pivot & \cellcolor{scorelo}0.032 & \cellcolor{scorelo}0.097 & +0.065 \\
\texttt{SleepPivot} & Pivot & \cellcolor{scorelo}0.091 & \cellcolor{scorehi}0.636 & +0.545 \\
\midrule
\texttt{BrowserRank} & Top-$k$ & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\midrule
\textbf{Passed ($\geq 0.5$)} & & \textbf{1/10} & \textbf{2/10} & \\
\textbf{Average score} & & \textbf{0.104} & \textbf{0.207} & \\
\bottomrule
\end{tabularx}
\caption{Conditional PE v1 with DP-noised aggregates ($\varepsilon_{\text{agg}}=1.0 + \varepsilon_{\text{hist}}=3.0 = 4.0$ total). The ``PE base'' column reports the midscale PE run (5{,}000 records, independent generation, same $\varepsilon=4.0$). Conditional generation improves on \texttt{RamHist} and \texttt{SleepPivot} where per-stratum conditioning aligns with query structure, but regresses on \texttt{BrowserHist} ($0.667 \to 0.000$) and \texttt{XeonGeo} ($0.250 \to 0.000$) because the high per-query noise ($\sigma \approx 150$) in v1 destroys the aggregate targets for those groups.}
\label{tab:conditional-pe}
\end{table}

Conditional v1 passes 2/10 queries (avg 0.207) versus the baseline's 1/10 (avg 0.104). Gains concentrate where query structure matches the conditioning: \texttt{RamHist} achieves a perfect 1.0 and \texttt{SleepPivot} passes at 0.636. However, v1 regresses on \texttt{BrowserHist} ($0.667 \to 0.000$) and \texttt{XeonGeo} ($0.250 \to 0.000$) because the high $\sigma \approx 150$ destroys the aggregate targets. Country-conditioned queries remain at 0 because per-country populations (1--128 systems) fall below the noise floor.

\subsubsection{Improved noise calibration}

A follow-up iteration addressed the root cause of the 2/10 result, where the per-query noise $\sigma \approx 150$ overwhelmed all but the largest strata. Four changes were applied simultaneously:

\begin{enumerate}
    \item Per-group budget splitting, partitioning the $k=1{,}056$ aggregate queries into 7 groups and allocating each group an equal share of the total zCDP $\rho$ budget.
    \item zCDP composition~\cite{bun2016concentrated}, replacing basic advanced composition with tighter accounting via $\varepsilon = \rho + 2\sqrt{\rho \ln(1/\delta)}$.
    \item Budget rebalancing from the histogram to aggregates ($\varepsilon_{\text{agg}} = 3.5$, $\varepsilon_{\text{hist}} = 0.5$, total unchanged at 4.0).
    \item Post-processing consistency, normalizing webcat percentage targets to sum to 100 and clamping noisy counts to $[0, \infty)$ (free under DP).
\end{enumerate}

The combined effect reduces per-query $\sigma$ from ${\sim}150$ to ${\sim}25\text{--}68$ depending on group size.

\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}YYYY}
\toprule
\textbf{Query} & \textbf{Type} & \textbf{PE base} & \textbf{Cond v1} & \textbf{Cond v2} & \textbf{$\Delta$} \\
\midrule
\texttt{ChassisAgg} & Agg+Join & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\midrule
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & 0.000 \\
\texttt{BatteryGeo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\texttt{PowerGeo} & Geo/Demo & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\texttt{XeonGeo} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500 & +0.500 \\
\midrule
\texttt{BrowserHist} & Histogram & \cellcolor{scorehi}0.667 & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & +0.333 \\
\texttt{RamHist} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & 0.000 \\
\midrule
\texttt{PersonaPivot} & Pivot & \cellcolor{scorelo}0.032 & \cellcolor{scorelo}0.097 & \cellcolor{scorelo}0.161 & +0.064 \\
\texttt{SleepPivot} & Pivot & \cellcolor{scorelo}0.091 & \cellcolor{scorehi}0.636 & \cellcolor{scorehi}0.636 & 0.000 \\
\midrule
\texttt{BrowserRank} & Top-$k$ & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & 0.000 \\
\midrule
\textbf{Passed ($\geq 0.5$)} & & \textbf{1/10} & \textbf{2/10} & \textbf{3/10} & \\
\textbf{Average score} & & \textbf{0.104} & \textbf{0.207} & \textbf{0.296} & \\
\bottomrule
\end{tabularx}
\caption{Conditional PE improvement: ``PE base'' is the midscale PE run (5{,}000 records, independent generation, $\varepsilon=4.0$); ``Cond v1'' uses basic advanced composition ($\varepsilon_{\text{agg}}=1.0$, $\sigma \approx 150$); ``Cond v2'' uses zCDP with per-group budget splitting ($\varepsilon_{\text{agg}}=3.5$, $\sigma \approx 25\text{--}68$). The $\Delta$ column shows v2 minus v1. All runs maintain $\varepsilon_{\text{total}}=4.0$.}
\label{tab:conditional-pe-v2}
\end{table}

The improved calibration lifts performance from 2/10 (v1) to 3/10 (v2) passed queries (avg 0.207 $\to$ 0.296). \texttt{XeonGeo} newly passes at 0.500 thanks to the reduced $\sigma_{\text{net}}=24.6$, and \texttt{BrowserHist} recovers to 0.333. Country-conditioned queries remain at 0 as per-country populations (1--128 systems) are still below the noise floor. Overall, conditional PE v2 nearly triples the baseline's average score (0.104 $\to$ 0.296), confirming that noise calibration is the binding constraint on utility.

\subsection{Cross-method comparison}

Across methods, pass counts are similar for per-table DP-SGD and MST, while wide-table DP-SGD and PE evaluate fewer queries because of schema coverage limits (Table~\ref{tab:method-comparison}). The conditional PE v2 result demonstrates that conditioning on DP-noised aggregates can improve PE utility on the subset of queries it covers.
...
Conditional PE v2 & 10 & 3 & 30.0\% & 0.296 & 0.247 \\
\bottomrule
\end{tabular*}
...
\midrule
Aggregate & Mean RE $\downarrow$ & 55.43 & \cellcolor{scorehi}0.54 & \cellcolor{scorelo}169.29 & \cellcolor{scoremid}11.84 \\
Distribution & Mean TV $\downarrow$ & \cellcolor{scorelo}0.279 & 0.241 & \cellcolor{scorehi}0.105 & \cellcolor{scoremid}0.184 \\
Histogram & Mean TV/RE $\downarrow$ & 0.744 & \cellcolor{scoremid}0.444 & \cellcolor{scorelo}1.000 & \cellcolor{scorehi}0.241 \\
Categorical ranking & Accuracy $\uparrow$ & 0.824 & \cellcolor{scoremid}0.922 & \cellcolor{scorehi}0.941 & \cellcolor{scorelo}0.255 \\
Numeric ranking & Spearman $\rho$ $\uparrow$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.172 & \cellcolor{scorehi}0.463 & \cellcolor{scorena}--- \\
Row-level & Mixed $\downarrow$ & \cellcolor{scorena}--- & \cellcolor{scorehi}1.192 & \cellcolor{scorelo}3.985 & \cellcolor{scorena}--- \\
\bottomrule
\end{tabular*}
\caption{Mean continuous metric values by query type, color-coded: \tightlegendbox{scorehi}{best}, \tightlegendbox{scoremid}{middle}, \tightlegendbox{scorelo}{worst} per row. Arrows indicate metric direction ($\downarrow$ = lower is better, $\uparrow$ = higher is better). \tightlegendbox{scorena}{N/A} means no queries of that type were evaluable for the method.}
\label{tab:continuous-metrics}
\end{table}

Per-table DP-SGD achieves the lowest aggregate RE (0.54), while MST leads on distributional fidelity (TV = 0.105) and ranking accuracy. Wide-table DP-SGD and PE evaluate fewer query types due to schema coverage limits but show competitive performance on histograms and distributions respectively. The high aggregate RE for MST (169.29) and wide-table (55.43) is driven by outlier queries with extreme numeric errors, suggesting these methods struggle with count preservation in joined aggregations.
...
\midrule
Per-table DP-SGD & 12.59 & \textbf{2.81} & 28.44 & 82 \\
Private Evolution & 34.07 & 8.28 & 92.50 & 47 \\
MST baseline & 1{,}756.36 & 9.02 & 13{,}235.52 & 83 \\
Wide-table DP-VAE & 280.54 & 16.61 & 1{,}116.38 & 50 \\
\bottomrule
\end{tabular*}
\caption{Normalized error summary across synthesis methods ($\hat{\Delta} = \Delta_j(D, \tilde{D}) / |\Delta_j(D_1, D_2)|$, lower is better). Metrics with zero baseline variation are excluded (see text). Per-table DP-SGD achieves the best median normalized error (2.81), meaning its typical metric-level distortion is under 3$\times$ the natural half-sample variation. Bold indicates best.}
\label{tab:normalized-error}
\end{table}

Per-table DP-SGD achieves the best median normalized error (2.81), followed by PE (8.28), MST (9.02), and wide-table DP-VAE (16.61). This ranking differs from the binary pass-rate ranking, where per-table and MST were tied at 28.6\%. The normalized metric reveals that per-table DP-SGD's errors are consistently smaller relative to natural variation, while MST's high mean (1{,}756) is driven by extreme outliers on aggregate queries that involve multi-table joins (e.g., \texttt{SleepPivot} at 12{,}368$\times$ and \texttt{DisplayAgg} at 4{,}430$\times$). Metrics where the baseline variation $|\Delta_j(D_1,D_2)|$ is below $10^{-8}$ (i.e., the two random halves produce identical values) are excluded, since they offer no natural variation to normalize against.

Table~\ref{tab:normalized-error-per-query} provides per-query normalized errors for the four base methods (conditional PE v2 is excluded because its evaluation detail data was not retained for normalized error computation).

\begin{footnotesize}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.11\linewidth}*{4}{>{\centering\arraybackslash}p{0.11\linewidth}}}
\caption{Per-query mean normalized error ($\hat{\Delta}$, lower is better), color-coded: \tightlegendbox{scorehi}{best}, \tightlegendbox{scoremid}{middle}, \tightlegendbox{scorelo}{worst} per row. \tightlegendbox{scorena}{N/A} means the method did not produce results for that query.}
...
\bottomrule
\endlastfoot
\texttt{BlockerAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}22.6 & \cellcolor{scorelo}57.8 & \cellcolor{scorena}--- \\
\texttt{BlockerDurAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}2.4 & \cellcolor{scorelo}45.4 & \cellcolor{scorena}--- \\
\texttt{ChassisAgg} & Agg+Join & \cellcolor{scorelo}1{,}481.5 & \cellcolor{scorena}--- & \cellcolor{scorena}--- & \cellcolor{scorehi}3.6 \\
\texttt{DisplayAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}12.7 & \cellcolor{scorelo}4{,}429.5 & \cellcolor{scorena}--- \\
\texttt{NetAsymAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}18.7 & \cellcolor{scorelo}24.0 & \cellcolor{scorena}--- \\
\texttt{VendorAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}34.1 & \cellcolor{scorehi}21.1 & \cellcolor{scorena}--- \\
\midrule
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorena}--- & \cellcolor{scorehi}14.4 & \cellcolor{scorelo}369.4 & \cellcolor{scorena}--- \\
\texttt{BatteryGeo} & Geo/Demo & \cellcolor{scoremid}294.7 & \cellcolor{scorehi}3.1 & \cellcolor{scorelo}356.7 & \cellcolor{scoremid}6.5 \\
\texttt{PowerGeo} & Geo/Demo & \cellcolor{scorelo}920.5 & \cellcolor{scoremid}3.7 & \cellcolor{scorehi}1.2 & \cellcolor{scoremid}9.4 \\
\texttt{XeonGeo} & Geo/Demo & \cellcolor{scoremid}60.4 & \cellcolor{scoremid}3.8 & \cellcolor{scorelo}211.8 & \cellcolor{scorehi}2.9 \\
\midrule
\texttt{BrowserHist} & Histogram & \cellcolor{scorelo}58.7 & \cellcolor{scoremid}46.0 & \cellcolor{scorehi}26.3 & \cellcolor{scoremid}49.5 \\
\texttt{RamHist} & Histogram & \cellcolor{scoremid}31.6 & \cellcolor{scoremid}37.8 & \cellcolor{scorelo}107.9 & \cellcolor{scorehi}14.7 \\
\midrule
\texttt{PersonaPivot} & Pivot & \cellcolor{scorelo}77.6 & \cellcolor{scorehi}4.6 & \cellcolor{scoremid}28.4 & \cellcolor{scoremid}45.7 \\
\texttt{SleepPivot} & Pivot & \cellcolor{scorena}--- & \cellcolor{scorehi}23.8 & \cellcolor{scorelo}12{,}367.9 & \cellcolor{scorena}--- \\
\midrule
\texttt{BrowserRank} & Top-$k$ & \cellcolor{scoremid}9.0 & \cellcolor{scoremid}4.0 & \cellcolor{scorehi}3.0 & \cellcolor{scorelo}38.0 \\
\texttt{DetectionRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}7.3 & \cellcolor{scorehi}2.8 & \cellcolor{scorena}--- \\
\texttt{FocalRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}0.8 & \cellcolor{scorelo}1.1 & \cellcolor{scorena}--- \\
\texttt{SystemRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}53.2 & \cellcolor{scorehi}5.0 & \cellcolor{scorena}--- \\
\texttt{WaitModeRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}0.9 & \cellcolor{scorehi}0.9 & \cellcolor{scorena}--- \\
\texttt{WaitRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}1.0 & \cellcolor{scorehi}1.0 & \cellcolor{scorena}--- \\
\texttt{WaitStateRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}1.0 & \cellcolor{scorehi}1.0 & \cellcolor{scorena}--- \\
\end{longtable}
\end{footnotesize}

The per-query breakdown reveals several patterns. First, per-table DP-SGD achieves normalized errors below 5$\times$ on 10 of 20 queries, indicating that for half of the benchmark its synthesis error is within a small multiple of natural sampling variation. Second, MST excels on ranking queries (\texttt{WaitRank}, \texttt{DetectionRank}) where it achieves near-unity normalized errors, confirming its strength in preserving marginal orderings. Third, the wide-table methods show extreme normalized errors on aggregate queries (e.g., \texttt{PowerGeo} at 921$\times$, \texttt{ChassisAgg} at 1{,}482$\times$), driven by the zero-inflation problem discussed in Section~\ref{sec:methods-wide-dpsgd}. Fourth, PE achieves competitive normalized errors on the queries it can evaluate (e.g., \texttt{ChassisAgg} at 3.6, \texttt{XeonGeo} at 2.9) but covers only 8 of 21 queries.
...
% \begin{footnotesize}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.10\linewidth}*{5}{>{\centering\arraybackslash}p{0.10\linewidth}}}
\caption{Per-query scores for all five synthesis methods, color-coded by fidelity: \tightlegendbox{scorehi}{$\geq 0.5$} (pass), \tightlegendbox{scoremid}{$0.25$--$0.49$}, \tightlegendbox{scorelo}{$< 0.25$}, \tightlegendbox{scorena}{N/A}. ``Cond PE'' is the conditional PE v2 run (Section~\ref{sec:pe-results}).}
\label{tab:per-query-scores} \\
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-tab} & \textbf{MST} & \textbf{PE} & \textbf{Cond PE} \\
\midrule
\endfirsthead
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-tab} & \textbf{MST} & \textbf{PE} & \textbf{Cond PE} \\
...
\bottomrule
\endlastfoot
\texttt{BlockerAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{BlockerDurAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{ChassisAgg} & Agg+Join & \cellcolor{scorelo}0.167 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{DisplayAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{NetAsymAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.143 & \cellcolor{scoremid}0.429 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{VendorAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scoremid}0.333 \\
\texttt{BatteryGeo} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}0.500 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{PowerGeo} & Geo/Demo & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{XeonGeo} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500 \\
\texttt{BrowserHist} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & \cellcolor{scorehi}0.667 & \cellcolor{scorehi}0.667 & \cellcolor{scoremid}0.333 \\
\texttt{RamHist} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500 & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500 & \cellcolor{scorehi}1.000 \\
\texttt{PersonaPivot} & Pivot & \cellcolor{scorelo}0.065 & \cellcolor{scoremid}0.419 & \cellcolor{scorelo}0.194 & \cellcolor{scorelo}0.032 & \cellcolor{scorelo}0.161 \\
\texttt{SleepPivot} & Pivot & \cellcolor{scorena}--- & \cellcolor{scorehi}0.636 & \cellcolor{scoremid}0.273 & \cellcolor{scorena}--- & \cellcolor{scorehi}0.636 \\
\texttt{BrowserRank} & Top-$k$ & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{DetectionRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}0.500 & \cellcolor{scorehi}1.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{FocalRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{SystemRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{WaitModeRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{WaitRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\texttt{WaitStateRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}--- & \cellcolor{scorena}--- \\
\end{longtable}
% \end{footnotesize}
...
\begin{axis}[
    ...
    ylabel={Queries passed},
        symbolic x coords={Wide,Per-tab,MST,PE,Cond PE},
        xtick={Wide,Per-tab,MST,PE,Cond PE},
        ymin=0, ymax=9,
    ytick={0,2,4,6,8},
    nodes near coords,
...
\addplot[fill=BrickRed!40, draw=BrickRed, bar shift=0pt] coordinates {(Per-tab,6)};
...
\addplot[fill=violet!40, draw=violet, bar shift=0pt] coordinates {(Cond PE,3)};
\end{axis}
\end{tikzpicture}
\end{minipage}
\hfill
...
\addplot[fill=BrickRed!40, draw=BrickRed, bar shift=0pt] coordinates {(Per-tab,0.303)};
...
\addplot[fill=violet!40, draw=violet, bar shift=0pt] coordinates {(Cond PE,0.296)};
\end{axis}
\end{tikzpicture}
\end{minipage}
\caption{Left: number of queries passing (score $\geq 0.5$) for each method. Right: average score across all evaluated queries. The per-table methods evaluate all 21 queries; wide-table and PE evaluate 8; conditional PE evaluates 10.}
\label{fig:method-comparison}
\end{figure}

The per-query scores in Table~\ref{tab:per-query-scores} show that no method achieves high scores on aggregate queries involving continuous metrics, while categorical and ranking queries show more variation across methods. Conditional PE v2 is strongest on histogram queries (\texttt{RamHist} at 1.000) and pivot queries (\texttt{SleepPivot} at 0.636), complementing the baseline PE's strengths.
...
\begin{axis}[
    ...
    legend style={at={(0.02,0.96)}, anchor=north west, font=\scriptsize},
    enlarge x limits=0.15,
    x tick label style={font=\small},
    grid=major,
...
\addplot[fill=violet!40, draw=violet] coordinates {
    (Agg+Join,0.000) (Top-$k$,0.000) (Geo/Demo,0.208) (Histogram,0.666) (Pivot,0.399)
};
\legend{Wide, Per-table, MST, PE, Cond PE}
\end{axis}
\end{tikzpicture}
...
We ran two additive experiments to test whether lightweight interventions can improve utility without changing the original synthesis methods.
...
There are clear differences in resource costs between methods. The wide-table DP-VAE has no external API bill, but it is the most compute intensive option and requires 360 minutes on CPU (20 epochs, 1M rows, 307 features). Per-table synthesis also avoids APIs and completes in approximately 90 minutes total (two VAEs at $\sim$30 minutes each, plus histogram synthesis for 17 smaller tables). MST is the lightest local compute path in this study and runs in under 10 minutes per table. PE is the only method with explicit out-of-pocket cost because it depends on a paid foundation-model API. It completes in 59 minutes total, where the OpenAI Batch API generates 150{,}000 candidate records (7{,}500 batch calls at approximately \$20 USD), and the DP nearest-neighbor histogram over 1{,}000{,}000 real $\times$ 150{,}000 synthetic records accounts for the remaining computation.

\subsubsection{Overall interpretation}

The PE results confirm that foundation models import their own distributional priors rather than learning the target distribution (Win11 at 77.8\% vs.\ 10.4\% real). The DP nearest-neighbor histogram partially corrects these biases for histogram-type queries (PE matches MST on browser usage at 0.667 and outperforms the wide-table DP-VAE on RAM utilization at 0.500 vs.\ 0.000), but cannot compensate for the near-total absence of nonzero continuous metrics in the LLM output. Although structured output constraints eliminate out-of-vocabulary categorical values, the distributional mismatch between the foundation model's prior and the real data still reduces the effective candidate pool for the nearest-neighbor histogram. This aligns with \citeauthor{swanberg2025apiaccessllmsuseful}'s finding that LLMs capture 1-way marginals reasonably but are inaccurate on $k$-way marginals.

The conditional PE experiments (Section~\ref{sec:pe-results}) show that conditioning LLM generation on DP-noised aggregate targets can partially close this gap. By providing the model with noisy per-stratum statistics, conditional PE v2 lifts the pass rate from 1/10 to 3/10 and the average score from 0.104 to 0.296 on the 10-query subset, with \texttt{RamHist} achieving a perfect 1.0. However, the approach remains limited by DP noise on small groups: country-conditioned queries score 0 because per-country populations (1--128 systems) fall below the noise floor even with improved calibration ($\sigma \approx 25\text{--}68$). This confirms that DP noise calibration, not model capacity, is the binding constraint on conditional PE utility.
...
\begin{itemize}
    ...
    \item Further conditional PE improvements such as reducing the number of aggregate queries per group, using adaptive budget allocation based on group size, or fundamentally different approaches to country-level conditioning.
\end{itemize}
...
We presented an evaluation framework for differentially private synthetic data generation on Intel's DCA telemetry corpus, implementing four base synthesis approaches---a wide-table DP-VAE trained with DP-SGD, per-table DP histogram synthesis, MST marginal-based synthesis, and Private Evolution (PE) via foundation model APIs---plus a conditional PE variant that conditions generation on DP-noised aggregate targets.

All methods have completed evaluation against the benchmark. Under binary pass-rate scoring, per-table DP-SGD and MST each pass 6 of 21 queries (28.6\%), while the wide-table DP-VAE passes 1 of 8 (12.5\%), PE passes 2 of 8 (25.0\%), and conditional PE v2 passes 3 of 10 (30.0\%). The conditional PE v2 result demonstrates that conditioning on DP-noised aggregates with zCDP composition and per-group budget splitting can nearly triple the baseline PE's average score (0.104 $\to$ 0.296) while maintaining the same total privacy budget ($\varepsilon = 4.0$). However, the normalized error metric (Section~\ref{sec:normalized-error}), which calibrates each query's synthesis error against the natural variation from a 50/50 data split, reveals a clearer ranking among the base methods: per-table DP-SGD achieves a median normalized error of 2.81$\times$, meaning its typical distortion is under 3 times the variation expected from halving the real dataset. PE follows at 8.28$\times$, MST at 9.02$\times$, and wide-table DP-VAE at 16.61$\times$. This continuous metric breaks the binary tie between per-table and MST, showing per-table's errors are consistently smaller relative to natural variation. Per-table DP-SGD achieves normalized errors below 5$\times$ on 10 of 20 queries, while MST excels specifically on ranking queries (achieving near-unity normalized errors on \texttt{WaitRank} and \texttt{WaitModeRank}) but suffers extreme errors on multi-table aggregate joins.
...
Our PE results are consistent with \citet{swanberg2025apiaccessllmsuseful}, and do not indicate a tabular advantage from foundation model API access in this setting. The broader lesson is that structure in the data representation and workload alignment matter more than model class alone.
