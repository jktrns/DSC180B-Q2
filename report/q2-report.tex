...
\midrule
Per-table DP-SGD & 14.00 & \textbf{2.34} & 33.34 & 84 \\
Private Evolution & 41.49 & 7.79 & 123.22 & 47 \\
MST baseline & 1{,}462.05 & 9.85 & 10{,}744.52 & 84 \\
Wide-table DP-VAE & 311.97 & 17.27 & 1{,}460.49 & 51 \\
\bottomrule
\end{tabular*}
\caption{Normalized error summary across synthesis methods ($\hat{\Delta} = \Delta_j(D, \tilde{D}) / |\Delta_j(D_1, D_2)|$, lower is better). Per-table DP-SGD achieves the best median normalized error (2.34), meaning its typical metric-level distortion is only 2.3$\times$ the natural half-sample variation. Bold indicates best.}
\label{tab:normalized-error}
\end{table}

Per-table DP-SGD achieves the best median normalized error (2.34), followed by PE (7.79), MST (9.85), and wide-table DP-VAE (17.27). This ranking differs from the binary pass-rate ranking, where per-table and MST were tied at 28.6\%. The normalized metric reveals that per-table DP-SGD's errors are consistently smaller relative to natural variation, while MST's high mean (1{,}462) is driven by extreme outliers on aggregate queries that involve multi-table joins (e.g., \texttt{OnOffMods} at 10{,}142$\times$ and \texttt{DisplayDurations} at 3{,}858$\times$).
...
\bottomrule
\endlastfoot
\texttt{XeonNet} & Agg & 57.1 & \textbf{3.9} & 614.4 & \textbf{3.0} \\
\texttt{ChassisAgg} & Agg & 2{,}044.0 & --- & --- & \textbf{3.7} \\
\texttt{BattDuration} & Agg & --- & \textbf{20.1} & 482.2 & --- \\
\texttt{BattGeo} & Agg & 156.6 & \textbf{2.7} & 334.9 & 12.5 \\
\texttt{DisplayDurations} & Agg & --- & \textbf{13.7} & 3{,}858.1 & --- \\
\texttt{DisplayVendors} & Dist & --- & \textbf{46.0} & 28.5 & --- \\
\texttt{ModsBlockers} & Agg & --- & \textbf{16.5} & 41.0 & --- \\
\texttt{BrowserCountry} & Cat & 9.0 & 4.0 & \textbf{3.0} & 38.0 \\
\texttt{OnOffMods} & Agg & --- & \textbf{20.4} & 10{,}141.6 & --- \\
\texttt{PersonaWeb} & Agg & 71.4 & \textbf{4.1} & 30.3 & 56.9 \\
\texttt{PkgPower} & Agg & 821.8 & \textbf{2.9} & \textbf{0.9} & 6.9 \\
\texttt{BrowserHist} & Dist & 68.1 & 60.7 & \textbf{27.3} & 55.6 \\
\texttt{RamHist} & Hist & 35.8 & 43.3 & 123.8 & \textbf{16.7} \\
\texttt{ServerExpl} & Row & --- & 38.5 & \textbf{49.6} & --- \\
\texttt{AppFocal} & Rank & --- & \textbf{0.8} & 1.1 & --- \\
\texttt{AppSysCount} & Rank & --- & 45.0 & \textbf{4.2} & --- \\
\texttt{AppDetections} & Rank & --- & 8.7 & \textbf{3.3} & --- \\
\texttt{ModsBlockerDur} & Agg & --- & \textbf{2.4} & 44.9 & --- \\
\texttt{UserWait10} & Rank & --- & \textbf{1.0} & \textbf{1.0} & --- \\
\texttt{UserWaitACDC} & Rank & --- & \textbf{0.9} & \textbf{0.9} & --- \\
\texttt{UserWait20} & Rank & --- & \textbf{1.0} & \textbf{1.0} & --- \\
\end{longtable}
\end{footnotesize}

The per-query breakdown reveals several patterns. First, per-table DP-SGD achieves normalized errors below 5$\times$ on 9 of 21 queries, indicating that for nearly half the benchmark its synthesis error is within a small multiple of natural sampling variation. Second, MST excels on ranking queries (\texttt{UserWait*}, \texttt{AppSysCount}) where it achieves near-unity normalized errors, confirming its strength in preserving marginal orderings. Third, the wide-table methods show extreme normalized errors on aggregate queries (e.g., \texttt{ChassisAgg} at 2{,}044$\times$), driven by the zero-inflation problem discussed in Section~\ref{sec:methods-wide-dpsgd}. Fourth, PE achieves competitive normalized errors on the queries it can evaluate (e.g., \texttt{XeonNet} at 3.0, \texttt{ChassisAgg} at 3.7) but covers only 8 of 21 queries.
