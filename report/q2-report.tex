...
\end{equation}
where the denominator $|\Delta_j(D_1, D_2)|$ is the natural sampling variation.\footnote{When $D_1$ and $D_2$ produce identical query results for a metric column ($|\Delta_j(D_1, D_2)| = 0$), the normalized error is undefined. We exclude such metrics (18 of 282, or 6.4\%, primarily Jaccard group-coverage scores) rather than using a stabilizer $\eta$, since an $\eta$-stabilized ratio would produce astronomically large values that dominate summary statistics.} A normalized error of $\hat{\Delta}_j = 1$ means the synthetic error equals the variation expected from halving the real dataset; $\hat{\Delta}_j < 1$ means the synthetic data is closer to the full real data than a random half-sample; $\hat{\Delta}_j \gg 1$ means the synthesis error far exceeds natural variation. The aggregate benchmark score is:
\begin{equation}
...
\midrule
Per-table DP-SGD & 15.45 & \textbf{2.72} & 39.70 & 82 \\
Private Evolution & 31.76 & 6.92 & 81.98 & 48 \\
MST baseline & 1{,}294.00 & 10.96 & 9{,}054.92 & 83 \\
Wide-table DP-VAE & 133.38 & 12.88 & 371.57 & 51 \\
\bottomrule
\end{tabular*}
\caption{Normalized error summary across synthesis methods ($\hat{\Delta} = \Delta_j(D, \tilde{D}) / |\Delta_j(D_1, D_2)|$, lower is better). Metrics with zero baseline variation are excluded (see text). Per-table DP-SGD achieves the best median normalized error (2.72), meaning its typical metric-level distortion is under 3$\times$ the natural half-sample variation. Bold indicates best.}
\label{tab:normalized-error}
\end{table}

Per-table DP-SGD achieves the best median normalized error (2.72), followed by PE (6.92), MST (10.96), and wide-table DP-VAE (12.88). This ranking differs from the binary pass-rate ranking, where per-table and MST were tied at 28.6\%. The normalized metric reveals that per-table DP-SGD's errors are consistently smaller relative to natural variation, while MST's high mean (1{,}294) is driven by extreme outliers on aggregate queries that involve multi-table joins (e.g., \texttt{SleepPivot} at 8{,}655$\times$ and \texttt{DisplayAgg} at 3{,}850$\times$). Metrics where the baseline variation $|\Delta_j(D_1,D_2)|$ is below $10^{-8}$ (i.e., the two random halves produce identical values) are excluded, since they offer no natural variation to normalize against.
...
\bottomrule
\endlastfoot
\texttt{BlockerAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}16.1 & \cellcolor{scorelo}35.3 & \cellcolor{scorena}--- \\
\texttt{BlockerDurAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}2.4 & \cellcolor{scorelo}45.1 & \cellcolor{scorena}--- \\
\texttt{ChassisAgg} & Agg+Join & \cellcolor{scorelo}309.3 & \cellcolor{scoremid}4.0 & \cellcolor{scoremid}4.0 & \cellcolor{scorehi}3.3 \\
\texttt{DisplayAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}13.7 & \cellcolor{scorelo}3{,}850.4 & \cellcolor{scorena}--- \\
\texttt{NetAsymAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}35.3 & \cellcolor{scorelo}46.9 & \cellcolor{scorena}--- \\
\texttt{VendorAgg} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}44.6 & \cellcolor{scorehi}27.5 & \cellcolor{scorena}--- \\
\midrule
\texttt{BatteryDemo} & Geo/Demo & \cellcolor{scorena}--- & \cellcolor{scorehi}12.6 & \cellcolor{scorelo}248.1 & \cellcolor{scorena}--- \\
\texttt{BatteryGeo} & Geo/Demo & \cellcolor{scorelo}139.1 & \cellcolor{scorehi}2.5 & \cellcolor{scorelo}320.5 & \cellcolor{scoremid}8.3 \\
\texttt{PowerGeo} & Geo/Demo & \cellcolor{scorelo}616.6 & \cellcolor{scoremid}3.3 & \cellcolor{scorehi}1.0 & \cellcolor{scoremid}8.3 \\
\texttt{XeonGeo} & Geo/Demo & \cellcolor{scorelo}96.5 & \cellcolor{scoremid}6.5 & \cellcolor{scorelo}593.4 & \cellcolor{scorehi}5.1 \\
\midrule
\texttt{BrowserHist} & Histogram & \cellcolor{scoremid}95.5 & \cellcolor{scorelo}118.0 & \cellcolor{scorehi}25.5 & \cellcolor{scoremid}65.1 \\
\texttt{RamHist} & Histogram & \cellcolor{scoremid}29.8 & \cellcolor{scoremid}38.3 & \cellcolor{scorelo}110.9 & \cellcolor{scorehi}14.5 \\
\midrule
\texttt{PersonaPivot} & Pivot & \cellcolor{scorelo}65.4 & \cellcolor{scorehi}3.4 & \cellcolor{scoremid}35.3 & \cellcolor{scorelo}41.2 \\
\texttt{SleepPivot} & Pivot & \cellcolor{scorena}--- & \cellcolor{scorehi}19.7 & \cellcolor{scorelo}8{,}655.3 & \cellcolor{scorena}--- \\
\midrule
\texttt{BrowserRank} & Top-$k$ & \cellcolor{scoremid}9.0 & \cellcolor{scoremid}4.0 & \cellcolor{scorehi}3.0 & \cellcolor{scorelo}38.0 \\
\texttt{DetectionRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}4.7 & \cellcolor{scorehi}2.0 & \cellcolor{scorena}--- \\
\texttt{FocalRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}1.0 & \cellcolor{scorelo}1.1 & \cellcolor{scorena}--- \\
\texttt{SystemRank} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}45.9 & \cellcolor{scorehi}9.7 & \cellcolor{scorena}--- \\
