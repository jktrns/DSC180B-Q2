...
PE remains weak on aggregate and geographic queries. The structured output constraint eliminates out-of-vocabulary categorical values, but coverage gaps persist: PE's candidate pool under-represents rare countries and demographic segments because the foundation model's prior favors common categories (e.g., the United States and Western European countries are over-represented relative to the DCA corpus). In \texttt{BrowserRank}, this coverage gap leads to poor overall score despite good local matches on the countries that are represented. \texttt{PersonaPivot} also remains low (0.032) because the persona distribution in PE output diverges from the real distribution.

\subsubsection{Stratified PE with DP-noised aggregates}
...
\toprule
\textbf{Query} & \textbf{Type} & \textbf{PE base} & \textbf{Strat v1} & \textbf{$\Delta$} \\
\midrule
\texttt{RamHist} & Histogram & 0.000 & \cellcolor{scorehi}1.000 & +1.000 \\
\texttt{BrowserHist} & Histogram & \cellcolor{scorehi}0.667 & 0.000 & $-$0.667 \\
\texttt{SleepPivot} & Pivot & 0.091 & \cellcolor{scorehi}0.636 & +0.545 \\
\texttt{XeonGeo} & Geo/Demo & 0.250 & 0.000 & $-$0.250 \\
\texttt{PersonaPivot} & Pivot & 0.032 & 0.097 & +0.065 \\
\texttt{BatteryDemo} & Geo/Demo & 0.000 & 0.333 & +0.333 \\
\texttt{BatteryGeo} & Geo/Demo & 0.000 & 0.000 & 0.000 \\
\texttt{ChassisAgg} & Agg+Join & 0.000 & 0.000 & 0.000 \\
\texttt{BrowserRank} & Top-$k$ & 0.000 & 0.000 & 0.000 \\
\texttt{PowerGeo} & Geo/Demo & 0.000 & 0.000 & 0.000 \\
\midrule
\textbf{Passed ($\geq 0.5$)} & & \textbf{1/10} & \textbf{2/10} & \\
\textbf{Average score} & & \textbf{0.104} & \textbf{0.207} & \\
\bottomrule
\end{tabularx}
\caption{Stratified PE v1 with DP-noised aggregates ($\varepsilon_{\text{agg}}=1.0 + \varepsilon_{\text{hist}}=3.0 = 4.0$ total). The ``PE base'' column reports the midscale PE run (5{,}000 records, independent generation, same $\varepsilon=4.0$). Stratified generation improves on \texttt{RamHist} and \texttt{SleepPivot} where per-stratum conditioning aligns with query structure, but regresses on \texttt{BrowserHist} ($0.667 \to 0.000$) and \texttt{XeonGeo} ($0.250 \to 0.000$) because the high per-query noise ($\sigma \approx 150$) in v1 destroys the aggregate targets for those groups.}
\label{tab:stratified-pe}
\end{table}

The baseline PE midscale run (5{,}000 records, independent generation at $\varepsilon = 4.0$) passes 1 of 10 evaluable queries with an average score of 0.104, with its only passing query being \texttt{BrowserHist} (0.667). The stratified v1 approach passes 2 of 10 queries with an average score of 0.207. The gains concentrate on queries whose structure matches the stratification: \texttt{RamHist} (stratified by memory size) achieves a perfect 1.0, and \texttt{SleepPivot} (stratified by CPU market code name) passes at 0.636. However, stratified v1 regresses on \texttt{BrowserHist} ($0.667 \to 0.000$) and \texttt{XeonGeo} ($0.250 \to 0.000$) because the high per-query noise ($\sigma \approx 150$) from composing all $k$ aggregate queries under a single budget destroys the aggregate targets for those groups. Country-stratified queries remain at 0 because the DP noise on per-country counts (sensitivity = 1 per system) overwhelms the small per-country populations, producing effectively uniform country distributions in the prompts.

This experiment demonstrates that conditioning PE generation on DP-noised aggregates can meaningfully improve utility for queries that align with the stratification, while maintaining formal privacy guarantees through proper composition.

\paragraph{Improved noise calibration.}
A follow-up iteration addressed the root cause of the 2/10 result: the per-query noise $\sigma \approx 150$ overwhelmed all but the largest strata. Four changes were applied simultaneously:

\begin{enumerate}
    \item \textbf{Per-group budget splitting:} Instead of composing all $k=1{,}056$ aggregate queries under a single $\varepsilon_{\text{agg}}$, we partition queries into 7 groups (battery, memory, on/off, browser/webcat, hardware, network, hints) and allocate each group an equal share of the total zCDP $\rho$ budget.
    \item \textbf{zCDP composition:} We replace basic advanced composition with zero-concentrated DP~\cite{bun2016concentrated}. We convert the total $(\varepsilon_{\text{agg}}, \delta)$ budget to a zCDP parameter $\rho_{\text{total}}$ via $\varepsilon = \rho + 2\sqrt{\rho \ln(1/\delta)}$, split $\rho$ equally across groups, then derive per-query $\sigma_g = \sqrt{k_g / (2\rho_g)}$ for each group $g$ with $k_g$ queries.
    \item \textbf{Budget rebalancing:} We shift budget from the histogram to aggregates: $\varepsilon_{\text{agg}} = 3.5$, $\varepsilon_{\text{hist}} = 0.5$ (total unchanged at 4.0). Since stratified generation already conditions the pool on targets, the histogram adds less marginal value than accurate aggregates.
    \item \textbf{Post-processing consistency:} After noising, webcat percentage targets are normalized to sum to 100 and all noisy counts are clamped to $[0, \infty)$. This is free under DP (deterministic post-processing).
\end{enumerate}

The combined effect reduces per-query $\sigma$ from ${\sim}150$ (global composition) to ${\sim}25\text{--}68$ depending on group size, a 2.3--6.4$\times$ improvement. Battery ($k=42$, $\sigma=25.2$), hardware ($k=48$, $\sigma=26.9$), and network ($k=40$, $\sigma=24.6$) benefit most because their small query counts receive the same $\rho$ budget as larger groups.

\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}YYYY}
\toprule
\textbf{Query} & \textbf{Type} & \textbf{PE base} & \textbf{Strat v1} & \textbf{Strat v2} & \textbf{$\Delta$} \\
\midrule
\texttt{RamHist} & Histogram & 0.000 & \cellcolor{scorehi}1.000 & \cellcolor{scorehi}1.000 & 0.000 \\
\texttt{BrowserHist} & Histogram & \cellcolor{scorehi}0.667 & 0.000 & 0.333 & +0.333 \\
\texttt{SleepPivot} & Pivot & 0.091 & \cellcolor{scorehi}0.636 & \cellcolor{scorehi}0.636 & 0.000 \\
\texttt{XeonGeo} & Geo/Demo & 0.250 & 0.000 & \cellcolor{scorehi}0.500 & +0.500 \\
\texttt{BatteryDemo} & Geo/Demo & 0.000 & 0.333 & 0.333 & 0.000 \\
\texttt{PersonaPivot} & Pivot & 0.032 & 0.097 & 0.161 & +0.064 \\
\texttt{BatteryGeo} & Geo/Demo & 0.000 & 0.000 & 0.000 & 0.000 \\
\texttt{ChassisAgg} & Agg+Join & 0.000 & 0.000 & 0.000 & 0.000 \\
\texttt{BrowserRank} & Top-$k$ & 0.000 & 0.000 & 0.000 & 0.000 \\
\texttt{PowerGeo} & Geo/Demo & 0.000 & 0.000 & 0.000 & 0.000 \\
\midrule
\textbf{Passed ($\geq 0.5$)} & & \textbf{1/10} & \textbf{2/10} & \textbf{3/10} & \\
\textbf{Average score} & & \textbf{0.104} & \textbf{0.207} & \textbf{0.296} & \\
\bottomrule
\end{tabularx}
\caption{Stratified PE improvement: ``PE base'' is the midscale PE run (5{,}000 records, independent generation, $\varepsilon=4.0$); ``Strat v1'' uses basic advanced composition ($\varepsilon_{\text{agg}}=1.0$, $\sigma \approx 150$); ``Strat v2'' uses zCDP with per-group budget splitting ($\varepsilon_{\text{agg}}=3.5$, $\sigma \approx 25\text{--}68$). The $\Delta$ column shows v2 minus v1. All runs maintain $\varepsilon_{\text{total}}=4.0$.}
\label{tab:stratified-pe-v2}
\end{table}

The improved calibration lifts performance from 2/10 (v1) to 3/10 (v2) passed queries (average 0.207 $\to$ 0.296). The newly passing query is \texttt{XeonGeo} ($0.500$), which benefits directly from the reduced $\sigma_{\text{net}}=24.6$ (vs.\ $\sigma=150$ in v1). \texttt{BrowserHist} also recovers to 0.333 in v2 (compared to 0.667 in the independent baseline and 0.000 in v1). Country-stratified queries remain at 0: even with $\sigma \approx 65$ for the hints group, the per-country populations (1--128 systems) are still below the noise floor. Overall, stratified PE v2 passes 3/10 queries at an average score of 0.296, compared to the independent baseline's 1/10 at 0.104. This confirms that DP noise calibration is the binding constraint on stratified PE utility, and further gains would require either reducing the number of queries per group or fundamentally different approaches to country-level conditioning.

\subsection{Cross-method comparison}

Across methods, pass counts are similar for per-table DP-SGD and MST, while wide-table DP-SGD and PE evaluate fewer queries because of schema coverage limits (Table~\ref{tab:method-comparison}). The stratified PE v2 result demonstrates that conditioning on DP-noised aggregates can improve PE utility on the subset of queries it covers.
...
Stratified PE v2 & 10 & 3 & 30.0\% & 0.296 & 0.247 \\
\bottomrule
\end{tabular*}
...
\midrule
Aggregate & Mean RE $\downarrow$ & 55.43 & \textbf{0.54} & 169.29 & 11.84 \\
Distribution & Mean TV $\downarrow$ & 0.279 & 0.241 & \textbf{0.105} & 0.184 \\
Histogram & Mean TV/RE $\downarrow$ & 0.744 & 0.444 & 1.000 & \textbf{0.241} \\
Categorical ranking & Accuracy $\uparrow$ & 0.824 & 0.922 & \textbf{0.941} & 0.255 \\
...
Table~\ref{tab:normalized-error-per-query} provides per-query normalized errors for the four base methods (stratified PE v2 is excluded because its evaluation detail data was not retained for normalized error computation).
...
\bottomrule
\endlastfoot
\texttt{BlockerAgg} & Agg+Join & --- & \textbf{16.1} & 35.3 & --- \\
\texttt{BlockerDurAgg} & Agg+Join & --- & \textbf{2.4} & 45.1 & --- \\
\texttt{ChassisAgg} & Agg+Join & 309.3 & 4.0 & 4.0 & \textbf{3.3} \\
\texttt{DisplayAgg} & Agg+Join & --- & \textbf{13.7} & 3{,}850.4 & --- \\
\texttt{NetAsymAgg} & Agg+Join & --- & \textbf{35.3} & 46.9 & --- \\
\texttt{VendorAgg} & Agg+Join & --- & 44.6 & \textbf{27.5} & --- \\
\midrule
\texttt{BatteryDemo} & Geo/Demo & --- & \textbf{12.6} & 248.1 & --- \\
\texttt{BatteryGeo} & Geo/Demo & 139.1 & \textbf{2.5} & 320.5 & 8.3 \\
\texttt{PowerGeo} & Geo/Demo & 616.6 & 3.3 & \textbf{1.0} & 8.3 \\
\texttt{XeonGeo} & Geo/Demo & 96.5 & 6.5 & 593.4 & \textbf{5.1} \\
\midrule
\texttt{BrowserHist} & Histogram & 95.5 & 118.0 & \textbf{25.5} & 65.1 \\
\texttt{RamHist} & Histogram & 29.8 & 38.3 & 110.9 & \textbf{14.5} \\
\midrule
\texttt{PersonaPivot} & Pivot & 65.4 & \textbf{3.4} & 35.3 & 41.2 \\
\texttt{SleepPivot} & Pivot & --- & \textbf{19.7} & 8{,}655.3 & --- \\
\midrule
\texttt{BrowserRank} & Top-$k$ & 9.0 & 4.0 & \textbf{3.0} & 38.0 \\
...
\caption{Per-query scores for all five synthesis methods, color-coded by fidelity: \tightlegendbox{scorehi}{$\geq 0.5$} (pass), \tightlegendbox{scoremid}{$0.25$--$0.49$}, \tightlegendbox{scorelo}{$< 0.25$}, \tightlegendbox{scorena}{N/A}. ``Strat PE'' is the stratified PE v2 run (Section~\ref{sec:pe-results}).}
\label{tab:per-query-scores} \\
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-tab} & \textbf{MST} & \textbf{PE} & \textbf{Strat PE} \\
\midrule
\endfirsthead
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-tab} & \textbf{MST} & \textbf{PE} & \textbf{Strat PE} \\
...
\begin{axis}[
    ...
    symbolic x coords={Wide,Per-tab,MST,PE,Strat PE},
    xtick={Wide,Per-tab,MST,PE,Strat PE},
    ymin=0, ymax=9,
    ytick={0,2,4,6,8},
    nodes near coords,
...
\addplot[fill=violet!40, draw=violet, bar shift=0pt] coordinates {(Strat PE,3)};
\end{axis}
\end{tikzpicture}
\end{minipage}
\hfill
...
\addplot[fill=violet!40, draw=violet, bar shift=0pt] coordinates {(Strat PE,0.296)};
\end{axis}
\end{tikzpicture}
\end{minipage}
\caption{Left: number of queries passing (score $\geq 0.5$) for each method. Right: average score across all evaluated queries. The per-table methods evaluate all 21 queries; wide-table and PE evaluate 8; stratified PE evaluates 10.}
\label{fig:method-comparison}
\end{figure}

The per-query scores in Table~\ref{tab:per-query-scores} show that no method achieves high scores on aggregate queries involving continuous metrics, while categorical and ranking queries show more variation across methods. Stratified PE v2 is strongest on histogram queries (\texttt{RamHist} at 1.000) and pivot queries (\texttt{SleepPivot} at 0.636), complementing the baseline PE's strengths.
...
\legend{Wide, Per-table, MST, PE, Strat PE}
\end{axis}
\end{tikzpicture}
...
The stratified PE experiments (Section~\ref{sec:pe-results}) show that conditioning LLM generation on DP-noised aggregate targets can partially close this gap. By providing the model with noisy per-stratum statistics, stratified PE v2 lifts the pass rate from 1/10 to 3/10 and the average score from 0.104 to 0.296 on the 10-query subset, with \texttt{RamHist} achieving a perfect 1.0. However, the approach remains limited by DP noise on small groups: country-stratified queries score 0 because per-country populations (1--128 systems) fall below the noise floor even with improved calibration ($\sigma \approx 25\text{--}68$). This confirms that DP noise calibration, not model capacity, is the binding constraint on stratified PE utility.
...
\begin{itemize}
    ...
    \item Further stratified PE improvements such as reducing the number of aggregate queries per group, using adaptive budget allocation based on group size, or fundamentally different approaches to country-level conditioning.
\end{itemize}
