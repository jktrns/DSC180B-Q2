...
\begin{enumerate}
    ...
    \item Add calibrated Gaussian noise to every aggregate query via the Gaussian mechanism, spending a privacy budget of $\varepsilon_{\text{agg}} = 1.0$. Per-query noise is calibrated via advanced composition: $\sigma_{\text{per-query}} = \sqrt{2k\ln(1.25/\delta)}/\varepsilon_{\text{agg}}$ where $k = 958$ is the total number of aggregate queries across all CSVs.
...
Stratified PE v2 & 10 & 3 & 30.0\% & 0.296 & 0.167 \\
\bottomrule
\end{tabular*}
